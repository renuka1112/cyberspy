  ...
  useEffect(() => {
    if (isConnected) {
      ...
      ws.current.onmessage = (event) => {
        ...
        setChartData(prev => [
          ...prev, 
          { time: new Date().toLocaleTimeString(), count: data.system.recv } // Use real network rx
        ].slice(-20));
      };
          ...
          <div className="glass-morphism p-6 rounded-3xl">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-mono text-cyber-green">24ms</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-400">Packet Loss</span>
                <span className="text-sm font-mono text-cyber-green">0.02%</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-400">Throughput</span>
                <span className="text-sm font-mono text-cyber-blue">1.2 GB/s</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-3 space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <AnimatePresence>
                  {threats.map((t) => (
                    <motion.div
                      ...
                      exit={{ opacity: 0, x: 20 }}
                      className="p-3 rounded-xl bg-white bg-opacity-5 border border-white border-opacity-5 flex justify-between items-center"
                    >
                      <div>
                        <div className="text-sm font-bold text-white uppercase tracking-tighter">{t.type}</div>
                        <div className="text-[10px] font-mono text-gray-500">{t.ip} • {t.timestamp}</div>
                      </div>
                      <div className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                        t.severity === 'Critical' ? 'bg-cyber-red text-white shadow-neon-red' : 
            ...
            <div className="glass-morphism p-6 rounded-3xl flex flex-col h-[500px]">
              ...
              <div className="mt-6 p-4 bg-cyber-green bg-opacity-10 rounded-2xl border border-cyber-green border-opacity-20 flex items-center gap-4">
                <CheckCircle2 className="text-cyber-green" />
                <div>
                  <div className="text-sm font-bold text-white">AI Engine Status</div>
                  <div className="text-xs text-cyber-green">Optimizing detection thresholds based on current stream volume.</div>
                </div>
              </div>
            </div>
          </div>
